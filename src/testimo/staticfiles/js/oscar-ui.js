...
var oscar = (function (o, $) {
    ...
    o.addressEvent = {
        ...
        load_address: function (address_data) {
            ...
            $('input.select2-search__field').css('height', '40px').css('font-size', "16px").css("font-size", "5px 5px");
    ...
    o.review = {
        ...
        reviewFirst: function (options) {
            const {url, token, ratingStar, ratingCalculate, page} = options;
            $.ajaxSetup({
                data: {
                    csrfmiddlewaretoken: token
                },
            ...
            $.ajax({
                ...
                success: function (json) {
                    if (json.status == "success") {
                        ...
                        const previous_page = json.previous_page;
                        const next_page = json.next_page;
                        $('#review-navbar').html(`
                            <div class="review-header d-block">
                                <div
                                    class="row between-lg between-xs between-sm middle-xs without-margin">
                                    <div class="review-arrow">
                                        <a class=${previous_page ? `review-arrow-slide` : `review-arrow-slide-disabled`} onclick=${previous_page ? `load_more_review(${previous_page})` : ``}>
                                    ...
                                    <div class="review-arrow">
                                        <a class=${next_page ? `review-arrow-slide` : `review-arrow-slide-disabled`}
                                                onclick=${next_page ? `load_more_review(${next_page})` : ``}>
                                            <i
        ...
        init: function (options) {
            ...
            $("#content_inner").on(
                ...
                function (event) {
                    ...
                    o.googleTagManager.removeCart($(this));
        ...
        init: function (options) {
            ...
            $('select.method-payment-chooose').on("change", function (event) {
                ...
                if (value.length > 0) {
                    ...
                    $('button#checkout-step[type="submit"]').prop('disabled', true);
                }
            });
        ...
        dropDown: function () {
            $(".gdn-dropdown-button").click(function (e) {
                ...
                if (dropdownContentElement.hasClass("active")) {
                    ...
                    dropdownContentElement.hide();
                } else {
                    if (activeElement.hasClass("active")) {
                        activeElement.hide();
                    }
                    dropdownContentElement.show();
    ...
    o.onScroll = {
        ...
        header: function (scrollTop) {
            ...
            if (windowsize > 980) {
                if (scrollTop > 20) {
                    $(".header-animation").addClass("header-scroll-animation");
                    $(".header .header-bottom").css('padding', '35px 0');
                    $(".gdn-logo-icon").addClass('active');
                    $(".gdn-logo-text").addClass('active');
                    $(".gdn-page").addClass("scroll");
                } else {
                    $(".header-animation").removeClass("header-scroll-animation");
                    $(".gdn-logo-icon").removeClass('active');
                    $(".gdn-logo-text").removeClass('active');
                    $(".header .header-bottom").css('padding', '52px 0');
    ...
    o.loading = {
        ...
        loadBarCreate: function () {
            ...
            var ovrl = o.loading.getElementLoading("loading-bar"),
                prog = o.loading.getElementLoading("loading-progress")
            img = document.images,
                c = 0;
    ...
    o.globalFunction = {
        ...
        onGetPrice(data) {
            return Number(data.split(",")[0].match(/\d/g).join(""))
        }
    };
    ...
    o.googleTagManager = {
        init: function (dataLayer) {
            $(document).ready(function () {
                //begin cart view
                o.googleTagManager.viewCart($(this));
                //end cart view
                if ($('#thankyoupage').length) {
                    o.googleTagManager.checkoutStepComplated();
                }
            });
            o.googleTagManager.checkoutStarted();
            o.googleTagManager.addToCart();
            o.googleTagManager.checkoutStep();
        },

        viewCart: function (elementDefault) {
            let elementCart = elementDefault.find("#basket_formset") || false;
            if (elementCart) {
                let data = elementCart.data("basket");
                let cart_id = elementCart.data("basketid");
                if (data) {
                    o.googleTagManager.pushEcommerce(dataLayer, {
                        'event': 'cartView',
                        'ecommerce': {
                            'currencyCode': 'Rp',
                            'view': {
                                "cart_id": cart_id,
                                'products': o.globalFunction.onGetJson(data) || []
                            }
                        }
                    })
                }
        ...
        removeCart: function (elementDefault) {
            let elementParent = elementDefault.parents(".basket-items") || false;
            let elementCart = elementDefault.find("#basket_formset") || false;

            if (elementParent && elementCart) {
                let cart_id = elementCart.data("basketid");
                o.googleTagManager.pushEcommerce(dataLayer, {
                    ...
                    'ecommerce': {
                        'remove': {
                            "cart_id": cart_id,
                            'products': [o.globalFunction.onGetJson(elementParent.data("product"))] || []
                        }
                    }
                });
            }
        ...
        checkoutStarted: function () {
            $("#checkout-started-basket").on("click", function (e) {
                const {onGetJson, onGetPrice} = o.globalFunction;
                let elementCart = $("#basket_formset");
                let cartTotal = $("#total-cart").data("total") || false;
                let data = elementCart.data("basket") || false;
                let cart_id = elementCart.data("basketid") || false;
                if (elementCart && data && cart_id) {
                    o.googleTagManager.pushEcommerce(dataLayer, {
                        'event': 'checkoutStarted',
                        'ecommerce': {
                            'remove': {
                                "cart_id": cart_id,
                                "affiliation": "testimo Store",
                                "value": onGetPrice(cartTotal),
                                "revenue": "",
                                "shipping": "",
                                "tax": "",
                                "discount": "",
        ...
        addToCart: function () {
            $("button#add-to-cart").on("click", function (e) {
                const {onGetJson} = o.globalFunction;
                let element = $("#product-detail").find("#product-detail-data") || false
                let productData = element.data("product") || false
                if (productData && element) {
                    let resultData = onGetJson(productData);
                    resultData.quantity = Number($("input[name=quantity]").val()) || 0
        ...
        checkoutStep: function () {
            $("#checkout-step").click(function (e) {
                const {onGetJson} = o.globalFunction;
                let elementBasket = $("#data-basket");
                let buttonStep = $("#checkout-step");
                let stepNumber = Number(buttonStep.data("step"));
                let cartId = elementBasket.data("basketid");
                if (elementBasket && buttonStep && elementBasket && stepNumber) {
                    let shippingMethod = stepNumber === 1 ? $("input[name=method_code]").val() : $(this).data("shipping");
                    let paymentMethod = stepNumber === 3 ? $("#checkout-step").data("payment") : false;
                    let isPayment = false;
                    if (stepNumber === 2) {
                        isPayment = true;
                        const elementVirtual = document.querySelector('input[name="payments"]:checked') || false;
                        const elementCc = document.querySelector('input[name="payment-method"]:checked') || false;

                        if (elementVirtual && elementVirtual.value === "virtual-account") {
                            let resultVa = $('#virtual-account-selected').find(":selected").text();
                            paymentMethod = resultVa === "Pilih Penyedia Virtual Account" ? false : resultVa;
                        }
                        if (elementCc && elementCc.value === "credit_card") {
                            let resultCC = $('#credit-card-select').find(":selected").text()
                            paymentMethod = resultCC === "Pilih Penyedia Virtual Account" ? false : resultCC;
                        }
                    }
                    if (stepNumber === 3) {
                        isPayment = true;
                    }
                    if (!paymentMethod && isPayment) {
                        return false;
                    }
                    ...
                    o.googleTagManager.pushEcommerce(dataLayer, {
                        ...
                        'ecommerce': {
                            'checkout': {
                                ...
                                "shipping_method": shippingMethod,
                                "payment_method": paymentMethod || "",
                                'products': onGetJson(elementBasket.data("basket"))
                            }
                        }
                    });
                }
        ...
        checkoutStepComplated: function () {
            const {onGetJson} = o.globalFunction;
            let elementBasket = $("#data-basket");
            let cartId = elementBasket.data("basketid");
            o.googleTagManager.pushEcommerce(dataLayer, {
        ...
        pushEcommerce: function (dataLayer, pushResult) {
            dataLayer.push(pushResult);
        }
    };
