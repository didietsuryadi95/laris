...
class BasketView(BasketCustomView):
    ...
    def is_voucher_available(self):
        ...
        for shipping_voucher in self.request.basket.shipping_discounts:
            self.request.basket.vouchers.remove(shipping_voucher.get('voucher'))
            voucher_removal.send(sender=self, basket=self.request.basket, voucher=shipping_voucher.get('voucher'))
            available = False

        return available

    def get(self, request, *args, **kwargs):
        if not self.is_voucher_available():
