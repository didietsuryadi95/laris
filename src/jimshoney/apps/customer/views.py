...
class MultipleFormsMixin(FormMixin):
    ...
    def get_forms_with_post(self, form_classes, action):
        return dict([(key, klass(**self.get_form_kwargs()))
                     if key == action else (key, klass(**{'user': self.request.user}))
                     for key, klass in form_classes.items()])
    ...
    def forms_invalid(self, forms):
        return super(MultipleFormsMixin, self).form_invalid(forms)
...
class ProfileView(PageTitleMixin, MultipleFormsView):
    ...
    def post(self, request, *args, **kwargs):
        form_classes = self.get_form_classes()
        action = self.request.POST['action']
        forms = self.get_forms_with_post(form_classes, action)
        form = forms.get(action, None)
        if form and form.is_valid():
            form.save()
...
def activate(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user, token):
        ...
        user.save()
        current_site = get_current_site(request)
        send_registration_activation_email.delay(user.id, scheme, current_site.domain)
