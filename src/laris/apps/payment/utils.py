...
from apps.templatetags.utils_tags import ipay_amount_formater
...
def ipay_signature_creator(order_number, total_order):
    ...
    ref_no = order_number
    amount = ipay_amount_formater(total_order)
    currency = getattr(settings, 'OSCAR_DEFAULT_CURRENCY')

    string_key = f'{merchant_key}{merchant_code}{ref_no}{amount}{currency}'
